...
const App: React.FC = () => {
  ...
  const initApp = async () => {
    try {
      ...
      // BOOTSTRAP: If DB is empty, or you are the only user, FORCE ADMIN
      ...
      // If profile is missing but user is logged in, attempt to auto-create it with CORRECT session ID
      if (!matchedProfile && session.user.email) {
        try {
          const newProfile = await apiService.addUser({
            id: sessionId, // Use actual Auth ID
          ...
          console.warn("Auto-creation failed, proceeding as session-only admin");
        }
      }
  ...
  const handleSaveTicket = async (data: Omit<DigTicket, 'id' | 'createdAt'>) => {
    try {
      ...
      alert(`Save Ticket Error: ${error.message}\n\nCheck if you are an Admin in the 'Profiles' table.`);
    }
  };
  ...
  const handleSaveJob = async (data: Omit<Job, 'id' | 'createdAt'>) => {
    try {
      ...
      alert(`Save Job Error: ${error.message}\n\nMake sure your 'Profiles' record has role='ADMIN'.`);
    }
  };
  ...
  if (isLoading) return (
    <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center">
      ...
      <p className="text-slate-500 font-black uppercase tracking-[0.4em] text-[10px]">Authorizing DB Access...</p>
    </div>
  );
            ...
            <div className="flex items-center gap-2 md:gap-4 ml-auto">
              ...
              {isAdmin ? (
                <div className="flex items-center gap-2">
                  <button onClick={() => { setEditingJob(null); setShowJobForm(true); }} className="bg-white text-[#0f172a] px-4 md:px-5 py-3 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center gap-2 shadow-xl shadow-white/5 text-[9px] md:text-[10px]">New Job</button>
                ...
                <div className="hidden md:block">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">Restricted Mode</span>
                </div>
              )}
            </div>
          </div>
