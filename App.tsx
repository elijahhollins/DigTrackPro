...
import LocatorOperationsDashboard from './components/LocatorOperationsDashboard.tsx';
...
const VALID_VIEWS: AppView[] = ['dashboard', 'operations', 'calendar', 'jobs', 'photos', 'team', 'map', 'asbuilt'];
...
const App: React.FC = () => {
  ...
  const handleAssignTicket = async (ticketId: string, crewId: string | null) => {
    const target = tickets.find(t => t.id === ticketId);
    if (!target) return;
    try {
      const updated: DigTicket = {
        ...target,
        assignedCrewId: crewId || undefined,
        assignedAt: crewId ? Date.now() : undefined,
      };
      const saved = await apiService.saveTicket(updated);
      setTickets(prev => prev.map(t => t.id === saved.id ? saved : t));
    } catch (error: any) {
  ...
  const NAV_ITEMS: { id: AppView; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'operations', label: 'Operations', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M8 15h8" /></svg> },
