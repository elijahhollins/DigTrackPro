  ...
  const handleSaveTicket = async (data: Omit<DigTicket, 'id' | 'createdAt'>, archiveOld: boolean = false) => {
    try {
      ...
      if (error.message?.includes("Requested entity was not found")) {
        setHasApiKey(false);
        if (confirm("AI connection lost. Would you like to reconnect your API key to continue parsing?")) {
           handleOpenSelectKey();
        }
      } else {
        alert(error.message);
