...
const App: React.FC = () => {
  ...
  const NAV_ITEMS: { id: AppView; label: string; icon: React.ReactNode }[] = [
    ...
    ...(isAdmin ? [{ id: 'billing' as AppView, label: 'Billing', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg> }] : []),
  ];
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <main key={activeView} className="flex-1 overflow-y-auto view-transition pb-20 sm:pb-0">
          <div className="max-w-[1400px] mx-auto px-5 py-6">
            ...
            {activeView === 'billing' && isAdmin && <BillingView companyId={sessionUser.companyId} companyName={company?.name ?? 'Your Company'} isDarkMode={isDarkMode} isSuperAdmin={isSuperAdmin} allCompanies={allCompanies} />}
          </div>
        </main>
      </div>
