...
const VALID_VIEWS: AppView[] = ['dashboard', 'calendar', 'jobs', 'photos', 'team', 'map', 'asbuilt', 'schedule', 'inventory', 'timetracking'];
...
const App: React.FC = () => {
  ...
  const handleToggleCompanyScheduling = async (id: string, schedulingEnabled: boolean) => {
    await apiService.setCompanySchedulingEnabled(id, schedulingEnabled);
    setAllCompanies(prev => prev.map(co => co.id === id ? { ...co, schedulingEnabled } : co));
    if (company?.id === id) setCompany({ ...company, schedulingEnabled });
  };

  const handleToggleCompanyTimeTracking = async (id: string, timeTrackingEnabled: boolean) => {
    await apiService.setCompanyTimeTrackingEnabled(id, timeTrackingEnabled);
    setAllCompanies(prev => prev.map(co => co.id === id ? { ...co, timeTrackingEnabled } : co));
    if (company?.id === id) setCompany({ ...company, timeTrackingEnabled });
  };
  ...
  const handleUpdateUserNotificationEmail = async (userId: string, email: string | null) => {
    await apiService.updateNotificationEmail(userId, email);
    setUsers(prev => prev.map(u => u.id === userId ? { ...u, notifyEmail: email || undefined } : u));
  };
  ...
  if (sessionUser.role !== UserRole.SUPER_ADMIN && company?.isActive === false) {
    return (
      <div className={`flex flex-col items-center justify-center min-h-screen gap-6 px-6 text-center ${isDarkMode ? 'bg-[#07101f] text-slate-100' : 'bg-slate-100 text-slate-900'}`}>
        ...
        <button
          ...
          className="px-6 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500/20 transition-all"
        >
          Sign Out
        </button>
      </div>
  ...
  const NAV_ITEMS: { id: AppView; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'photos', label: 'Photo', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg> },
                                  ...
                                  <tr key={ticket.id} data-ticket-id={ticket.id} onClick={() => isAdmin && setEditingTicket(ticket)} className={`group transition-all border-l-2 border-brand/25 ${isAdmin ? 'cursor-pointer' : ''} ${highlightedTicketId === ticket.id ? isDarkMode ? 'bg-brand/20 border-brand' : 'bg-brand/10 border-brand' : isDarkMode ? 'bg-black/20 hover:bg-white/[0.02]' : 'bg-slate-50/50 hover:bg-brand/5'} ${ticket.isArchived ? 'opacity-40' : ''}`}>
                                    <td className="px-5 py-3 pl-14">
                                      <div className="flex items-center gap-2.5">
                                        ...
                                        <button onClick={(e) => { e.stopPropagation(); window.open(`https://newtin.julie1call.com/responsedisplay/?ticket=${ticket.ticketNo}`, '_blank', 'noopener,noreferrer'); }} title="Check Positive Response" className={`flex items-center gap-1 px-2 py-1 rounded-lg transition-all shrink-0 text-[9px] font-black uppercase tracking-widest ${isDarkMode ? 'bg-brand/15 text-brand hover:bg-brand/30' : 'bg-brand/10 text-brand hover:bg-brand/25'}`}>
                                          <svg className="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" /></svg>
                                          Pos. Response
                                        </button>
