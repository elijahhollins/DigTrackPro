  ...
  const SortHeader = ({ field, label }: { field: SortField, label: string }) => {
    ...
    return (
      <th 
        ...
        className={`px-8 py-7 text-[10px] font-black uppercase tracking-[0.2em] cursor-pointer transition-colors group select-none ${isActive ? 'text-brand' : 'text-slate-500 hover:text-slate-300'}`}
      >
        <div className="flex items-center gap-1.5">
          {label}
          <div className={`transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-30'}`}>
            {sortConfig.order === 'asc' && isActive ? (
          ...
          <div className="space-y-8">
            ...
            <div className={`p-4 md:p-6 rounded-[2.5rem] border flex flex-col md:flex-row md:items-center justify-between gap-6 ${isDarkMode ? 'bg-[#1e293b] border-white/5' : 'bg-white border-slate-200 shadow-sm'}`}>
              <div className="flex-1 max-w-md relative lg:hidden">
                ...
                <svg className="w-4 h-4 text-slate-600 absolute left-3.5 top-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              </div>

              <div className="flex items-center gap-3 overflow-x-auto no-scrollbar pb-1 md:pb-0">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest hidden sm:inline whitespace-nowrap">Sort By:</span>
                {[
                  { field: 'jobNumber', label: 'Job #' },
                  { field: 'ticketNo', label: 'Ticket' },
                  { field: 'address', label: 'Address' },
                  { field: 'status', label: 'Status' },
                  { field: 'expirationDate', label: 'Expiry' }
                ].map((s) => (
                  <button 
                    key={s.field}
                    onClick={() => handleSort(s.field as SortField)}
                    className={`px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-[0.15em] border transition-all flex items-center gap-2 shrink-0 ${sortConfig.field === s.field ? 'bg-brand border-brand text-[#0f172a] shadow-lg shadow-brand/30 scale-105' : (isDarkMode ? 'bg-white/5 border-white/5 text-slate-400 hover:text-white' : 'bg-slate-50 border-slate-100 text-slate-600 hover:bg-slate-100')}`}
                  >
                    {s.label}
                    {sortConfig.field === s.field && (
                      <svg className={`w-3 h-3 transition-transform ${sortConfig.order === 'desc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 15l7-7 7 7" /></svg>
                    )}
                  </button>
                ))}
              </div>
