...
const DebugOverlay: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const capture = (type: string) => (...args: any[]) => {
      original[type as keyof typeof original](...args);
  ...
  if (!visible) return (
    <button
      ...
      style={{ position: 'fixed', bottom: 80, right: 16, zIndex: 9999, background: '#3b82f6', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 12px', fontSize: 11, fontWeight: 900 }}
    >
      DEBUG
    </button>
  );
  ...
  return (
    <div style={{ position: 'fixed', bottom: 80, left: 8, right: 8, maxHeight: '40vh', zIndex: 9999, background: 'rgba(0,0,0,0.92)', borderRadius: 12, border: '1px solid #334155', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', borderBottom: '1px solid #334155', shrink: 0 }}>
        <span style={{ color: '#94a3b8', fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Debug Console</span>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => setLogs([])} style={{ color: '#64748b', background: 'none', border: 'none', fontSize: 10, fontWeight: 900, cursor: 'pointer' }}>CLEAR</button>
          <button onClick={() => setVisible(false)} style={{ color: '#64748b', background: 'none', border: 'none', fontSize: 10, fontWeight: 900, cursor: 'pointer' }}>HIDE</button>
        </div>
      </div>
      <div style={{ overflowY: 'auto', padding: '8px 12px', flex: 1 }}>
        {logs.length === 0
          ? <p style={{ color: '#475569', fontSize: 10, margin: 0 }}>No logs yet — trigger a no-show to begin.</p>
          : logs.map((log, i) => (
            <p key={i} style={{
              margin: '2px 0', fontSize: 10, fontFamily: 'monospace', whiteSpace: 'pre-wrap', wordBreak: 'break-all',
              color: log.startsWith('[error]') ? '#f87171' : log.startsWith('[warn]') ? '#fbbf24' : '#4ade80'
            }}>{log}</p>
          ))
        }
      </div>
