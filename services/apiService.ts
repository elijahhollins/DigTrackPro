...
const mapInvItem = (d: Record<string, unknown>): InventoryItem => ({
  ...
  unitNumber: (d.unit_number as string) || undefined,
  equipmentType: (d.equipment_type as string) || undefined,
  ...
  hourlyRate: d.hourly_rate != null ? Number(d.hourly_rate) : undefined,
...
export const apiService = {
  ...
  async saveTicket(ticket: DigTicket, archiveExisting: boolean = false): Promise<DigTicket> {
    ...
    const { data, error } = await supabase.from('tickets').upsert({
      ...
      dig_by_date: ticket.digByDate || null,
      ...
      work_begun: ticket.workBegun ?? null,
