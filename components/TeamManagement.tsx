...
const TeamManagement: React.FC<TeamManagementProps> = ({ users, currentUserId, onAddUser, onDeleteUser, onThemeChange, onToggleRole }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-black text-slate-900 tracking-tight uppercase">User Registry</h2>
          <p className="text-xs text-slate-600 font-bold uppercase tracking-widest mt-1">Access Control & Permissions</p>
        </div>
        <button 
      ...
      {showAddForm && (
        <div className="bg-white p-8 rounded-3xl border border-brand/20 shadow-xl shadow-brand/5 animate-in slide-in-from-top-4 duration-300">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Default Password</label>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Security Role</label>
        ...
        <table className="w-full text-left">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="px-8 py-5 text-[10px] font-black text-slate-900 uppercase tracking-widest">Active Role</th>
              ...
              <tr key={user.id} className="hover:bg-slate-50 transition-colors group">
                <td className="px-8 py-5">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center font-black text-slate-500 text-xs border-2 border-white shadow-sm">{user.name.substring(0, 1)}</div>
                ...
                <td className="px-8 py-5">
                  <button 
                    ...
                    className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all hover:scale-105 ${user.role === UserRole.ADMIN ? 'bg-orange-50 text-orange-700 border-orange-200 shadow-sm' : 'bg-slate-100 text-slate-600 border-slate-200'}`}
                  >
                    {user.role}
                  </button>
                </td>
