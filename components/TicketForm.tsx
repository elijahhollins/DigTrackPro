        ...
        {!isEditing && (
          <div className="flex px-8 pt-6 gap-6 border-b border-slate-50">
            <button 
              ...
              className={`pb-4 text-[10px] font-black uppercase tracking-widest border-b-2 transition-all ${activeTab === 'manual' ? 'border-orange-600 text-orange-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
            >
              Manual Input
            </button>
            <button 
              onClick={() => setActiveTab('batch')}
              className={`pb-4 text-[10px] font-black uppercase tracking-widest border-b-2 transition-all ${activeTab === 'batch' ? 'border-orange-600 text-orange-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
            >
              AI Smart Scan
            </button>
          </div>
            ...
            <div className="space-y-8 pb-8">
              <div className="bg-orange-50/30 border-2 border-dashed border-orange-100 rounded-[2rem] p-12 text-center relative group hover:bg-orange-50/50 transition-all">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <h3 className="text-base font-black text-slate-900 uppercase tracking-tight">Upload Locate Documents</h3>
                  <p className="text-xs text-slate-500 mt-2 max-w-[320px] mx-auto leading-relaxed font-medium">Select PDF or Photos. Our AI will automatically extract job details for you.</p>
                </div>
              </div>

              {batchResults.length > 0 && (
                <div className="space-y-4">
                  <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Processing Queue</h4>
                    ...
                    {batchResults.map((res, i) => (
                      <div key={i} className="flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 shadow-sm">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs font-bold text-slate-700 truncate max-w-[150px]">{res.name}</span>
                        </div>
            ...
            <div className="space-y-8 pb-12">
                ...
                <div className="p-6 bg-orange-50/50 border border-orange-100 rounded-[2rem]">
                  ...
                  <div className="flex gap-3">
                    <textarea
                      ...
                      className="flex-1 text-xs p-4 border border-orange-100 rounded-2xl focus:ring-4 focus:ring-orange-100/50 focus:border-orange-400 transition-all outline-none resize-none h-20 bg-white shadow-inner text-slate-900 font-medium placeholder:text-slate-400"
                    />
                    <button
                      type="button"
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Job Site ID</label>
                  <input required type="text" className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-[1.25rem] text-sm font-semibold text-slate-900 focus:ring-4 focus:ring-orange-100/50 focus:bg-white outline-none transition-all placeholder:text-slate-300" value={formData.jobNumber} onChange={e => setFormData({...formData, jobNumber: e.target.value})} />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Ticket Number</label>
                  <input required type="text" className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-[1.25rem] text-sm font-semibold text-slate-900 focus:ring-4 focus:ring-orange-100/50 focus:bg-white outline-none transition-all placeholder:text-slate-300" value={formData.ticketNo} onChange={e => setFormData({...formData, ticketNo: e.target.value})} />
                </div>
                <div className="md:col-span-2 space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Work Site Address</label>
                  <input required type="text" className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-[1.25rem] text-sm font-semibold text-slate-900 focus:ring-4 focus:ring-orange-100/50 focus:bg-white outline-none transition-all placeholder:text-slate-300" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} />
                </div>
                
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Location Details</label>
                  <div className="flex gap-3">
                    <input type="text" placeholder="City" className="flex-1 px-5 py-4 bg-slate-50 border border-slate-100 rounded-[1.25rem] text-sm font-semibold text-slate-900 outline-none placeholder:text-slate-300" value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})} />
                    <input type="text" placeholder="ST" className="w-20 px-5 py-4 bg-slate-50 border border-slate-100 rounded-[1.25rem] text-sm font-semibold text-slate-900 text-center uppercase outline-none placeholder:text-slate-300" value={formData.state} onChange={e => setFormData({...formData, state: e.target.value.toUpperCase()})} />
                  </div>
                </div>

                <div className="space-y-2 relative" ref={suggestionRef}>
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Site Contact</label>
                  <input 
                    required 
                    type="text" 
                  ...
                  {showSuggestions && crewSuggestions.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                      <div className="p-2 border-b border-slate-50 bg-slate-50/50">
                        <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest ml-2">Crew Suggestions</span>
                      </div>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Call In Date</label>
                  <input required type="date" className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-[1.25rem] text-sm font-semibold text-slate-900 outline-none" value={formData.callInDate} onChange={e => setFormData({...formData, callInDate: e.target.value})} />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Dig Start</label>
                  <input required type="datetime-local" className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-[1.25rem] text-sm font-semibold text-slate-900 outline-none" value={formData.digStart} onChange={e => setFormData({...formData, digStart: e.target.value})} />
                </div>
                <div className="md:col-span-2 space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Hard Expiration Date</label>
                  <input required type="date" className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-[1.25rem] text-sm font-semibold text-slate-900 outline-none" value={formData.expirationDate} onChange={e => setFormData({...formData, expirationDate: e.target.value})} />
                </div>
