...
const PhotoManager: React.FC<PhotoManagerProps> = ({
  ...
  const handleBatchDelete = async () => {
    ...
    if (!confirm(`Permanently delete ${photoIds.length} photo${photoIds.length !== 1 ? 's' : ''}? Ticket documents in the selection will be skipped.`)) return;
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    if (e.key === 'ArrowRight') setLightboxIndex(i => (i !== null && i < currentAssets.length - 1) ? i + 1 : i);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 animate-in h-[calc(100vh-140px)] -mt-2 overflow-hidden">
      ...
      {/* SIDEBAR */}
      <aside className={`lg:w-64 xl:w-72 shrink-0 flex flex-col rounded-2xl border overflow-hidden ${d ? 'bg-slate-900 border-white/[0.08]' : 'bg-white border-slate-200 shadow-sm'}`}>
        <div className={`px-4 pt-4 pb-3 border-b ${d ? 'border-white/[0.08]' : 'border-slate-100'}`}>
          <p className={`text-[10px] font-semibold uppercase tracking-widest mb-3 ${d ? 'text-slate-500' : 'text-slate-400'}`}>Job Directory</p>
          <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${d ? 'bg-slate-800 border-white/[0.08]' : 'bg-slate-50 border-slate-200'}`}>
            <svg className={`w-3.5 h-3.5 shrink-0 ${d ? 'text-slate-500' : 'text-slate-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <input
              type="text"
              placeholder="Search jobs..."
              className={`flex-1 bg-transparent outline-none text-[12px] placeholder:text-slate-400 ${d ? 'text-white' : 'text-slate-900'}`}
              value={sidebarSearch}
              onChange={e => setSidebarSearch(e.target.value)}
            />
            {sidebarSearch && (
              <button onClick={() => setSidebarSearch('')} className={`transition-colors ${d ? 'text-slate-500 hover:text-white' : 'text-slate-400 hover:text-slate-600'}`}>
        ...
        <div className="flex-1 overflow-y-auto py-2 px-2 space-y-0.5">
          {filteredSidebarFolders.length === 0 ? (
            <p className={`text-center py-10 text-[11px] font-medium ${d ? 'text-slate-600' : 'text-slate-400'}`}>No jobs found</p>
            ...
            return (
              <div
                ...
                className={`group relative flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all ${
                  isActive ? 'bg-brand text-slate-900' : d ? 'text-slate-300 hover:bg-white/[0.06]' : 'text-slate-700 hover:bg-slate-50'
                }`}
                onClick={() => { setSelectedFolder(num); setActiveTab('all'); }}
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors ${isActive ? 'bg-black/10' : d ? 'bg-slate-800' : 'bg-slate-100'}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {jobEntity?.customer && (
                    <p className={`text-[10px] truncate leading-tight mt-0.5 ${isActive ? 'text-slate-900/60' : d ? 'text-slate-500' : 'text-slate-400'}`}>{jobEntity.customer}</p>
                  )}
                  <div className={`flex items-center gap-1.5 mt-1 ${isActive ? 'text-slate-900/50' : d ? 'text-slate-600' : 'text-slate-400'}`}>
                ...
                {isAdmin && jobEntity && onDeleteJob && (
                  <button
                    ...
                    className={`p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all shrink-0 ${isActive ? 'hover:bg-black/10 text-slate-900/50 hover:text-rose-700' : d ? 'hover:bg-rose-500/10 text-slate-600 hover:text-rose-400' : 'hover:bg-rose-50 text-slate-400 hover:text-rose-500'}`}
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {selectedFolder ? (
          <>
            {/* Header */}
            <div className={`shrink-0 mb-3 px-5 py-3.5 rounded-2xl border ${d ? 'bg-slate-900 border-white/[0.08]' : 'bg-white border-slate-200 shadow-sm'}`}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h2 className={`text-[15px] font-bold ${d ? 'text-white' : 'text-slate-900'}`}>Job #{selectedFolder}</h2>
                  ...
                  {selectedJob && (
                    <p className={`text-[11px] mt-0.5 truncate ${d ? 'text-slate-400' : 'text-slate-500'}`}>
                ...
                <div className="flex items-center gap-2 shrink-0 flex-wrap">
                  <div className={`flex items-center rounded-xl p-0.5 gap-0.5 ${d ? 'bg-slate-800' : 'bg-slate-100'}`}>
                  ...
                  <div className={`hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl border ${d ? 'bg-slate-800 border-white/[0.08] text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`}>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-xl border transition-all ${
                      selectionMode
                        ...
                        : d ? 'bg-slate-800 border-white/[0.08] text-slate-400 hover:text-white' : 'bg-slate-50 border-slate-200 text-slate-500 hover:text-slate-700'
                    }`}
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex-1 overflow-y-auto pb-10">
              {currentAssets.length > 0 ? (
                      ...
                      <div
                        ...
                        className={`group relative rounded-xl border overflow-hidden cursor-pointer transition-all duration-200 ${
                          isSelected
                            ...
                            : d ? 'bg-slate-800 border-white/[0.08] hover:border-white/20' : 'bg-white border-slate-200 hover:border-slate-300 shadow-sm hover:shadow-md'
                        }`}
                      >
                        <div className={`relative aspect-square flex items-center justify-center overflow-hidden ${d ? 'bg-slate-700' : 'bg-slate-100'}`}>
                          {isPdfAsset ? (
                            <div className={`flex flex-col items-center justify-center gap-2 w-full h-full ${d ? 'bg-slate-800' : 'bg-slate-50'}`}>
                              ...
                              <span className={`text-[9px] font-semibold uppercase tracking-wider ${d ? 'text-slate-500' : 'text-slate-400'}`}>PDF</span>
                            </div>
                          ) : (
                        ...
                        <div className={`px-2.5 py-2 ${d ? 'bg-slate-800' : 'bg-white'}`}>
                          <div className="flex items-center gap-1.5">
                            <span className={`inline-block w-1.5 h-1.5 rounded-full shrink-0 ${isTicketAsset ? 'bg-rose-500' : 'bg-brand'}`} />
                            <p className={`text-[11px] font-medium truncate flex-1 ${d ? 'text-slate-300' : 'text-slate-700'}`}>{asset.caption}</p>
                          </div>
                          <p className={`text-[10px] mt-0.5 ${d ? 'text-slate-600' : 'text-slate-400'}`}>{new Date(asset.timestamp).toLocaleDateString()}</p>
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className={`flex flex-col items-center justify-center h-64 rounded-2xl border-2 border-dashed text-center ${d ? 'border-white/[0.08] text-slate-600' : 'border-slate-200 text-slate-400'}`}>
          ...
          <div className={`flex-1 flex flex-col items-center justify-center text-center rounded-2xl border-2 border-dashed ${d ? 'border-white/[0.08] text-slate-500' : 'border-slate-200 text-slate-400'}`}>
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 ${d ? 'bg-slate-800' : 'bg-slate-100'}`}>
                  ...
                  <button
                    onClick={() => {
                      if (confirm('Permanently delete this photo?')) {
                        ...
                        const nextIndex = lightboxIndex >= currentAssets.length - 1
                          ? Math.max(0, lightboxIndex - 1)
                          : lightboxIndex;
                        setLightboxIndex(currentAssets.length <= 1 ? null : nextIndex);
                      }
                    }}
      ...
      {uploadQueue.length > 0 && (
        <div className="fixed bottom-24 right-4 w-72 z-[200] animate-in slide-in-from-right">
          <div className={`rounded-2xl border overflow-hidden shadow-xl ${d ? 'bg-slate-900 border-white/10' : 'bg-white border-slate-200'}`}>
            <div className={`flex items-center justify-between px-4 py-3 border-b ${d ? 'border-white/[0.08]' : 'border-slate-100'}`}>
              <div className="flex items-center gap-2.5">
                ...
                <p className={`text-[12px] font-semibold ${d ? 'text-white' : 'text-slate-900'}`}>{isUploading ? 'Uploading...' : 'Upload complete'}</p>
              </div>
              <button onClick={() => setUploadQueue([])} className={`p-1 rounded-lg transition-colors ${d ? 'text-slate-500 hover:text-white' : 'text-slate-400 hover:text-slate-600'}`}>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <div className="max-h-48 overflow-y-auto p-2 space-y-1.5">
              {uploadQueue.map(item => (
                <div key={item.id} className={`flex items-center gap-2.5 p-2 rounded-xl ${d ? 'bg-slate-800' : 'bg-slate-50'}`}>
                  <div className="w-9 h-9 rounded-lg overflow-hidden shrink-0">
                    <img src={item.previewUrl} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-[11px] font-medium truncate ${d ? 'text-slate-300' : 'text-slate-700'}`}>{item.file.name}</p>
                    <div className={`w-full h-1 rounded-full mt-1 overflow-hidden ${d ? 'bg-slate-700' : 'bg-slate-200'}`}>
