...
const StatCards = ({ tickets }: { tickets: DigTicket[] }) => {
  ...
  const items = [
    { label: 'Total Tickets', value: tickets.length, color: 'border-brand text-brand shadow-brand/10' },
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
      {items.map((it, idx) => (
        <div key={idx} className={`bg-white p-6 md:p-8 rounded-[2rem] border-2 ${it.color.split(' ')[0]} shadow-sm hover:shadow-xl transition-all`}>
