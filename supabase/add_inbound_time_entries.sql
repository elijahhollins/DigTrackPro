...
-- Index for fast per-ticket and per-tech look-ups
...
-- Admins (ADMIN / SUPER_ADMIN) can read all entries for their company's tickets
CREATE POLICY "admin_read_time_entries" ON inbound_ticket_time_entries
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles p
      JOIN inbound_tickets t ON t.id = inbound_ticket_time_entries.ticket_id
      WHERE p.id = auth.uid()
